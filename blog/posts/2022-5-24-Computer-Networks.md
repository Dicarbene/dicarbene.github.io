...
![tcp-3](/img/in-post/2022-05-24/tcp-3.png)

最初，客户端 A 和服务器 B 都处于 CLOSED 状态。然后 A 主动打开连接，B 被动打开连接。
...
![tcp-4](/img/in-post/2022-05-24/tcp-4.png)
...
- 然后 A 进入 `TIME-WAIT` 状态，经过 2 _ [MSL](#msl)（2 _ 2min = 4min）的时间后，A 进入 `CLOSED` 状态。而 B 只要收到 A 发出的确认，就立即进入 `CLOSED` 状态。所以 B 结束 TCP 连接的时间要早于 A
